<template>
  <ol class="step-bar">
    ...
    >
      <span class="num">{{ ui.saved[s.key] ? '✓' : s.id }}</span>
...
function onStepClick(key: StepKey): void {
  ...
  // Allow free navigation between visited or done steps; future steps are
  // gated behind the Next button to keep novices on the rails.
  if (ui.visited[key]) {
    ui.goTo(key);
  }
}

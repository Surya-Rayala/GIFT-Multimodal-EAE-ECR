// Tauri shell for the Scaled Point Viewer.
//
// Lifecycle:
//   1. On startup, spawn the Python sidecar
//      (`python -m src.utils.scaled_point_viewer.backend.sidecar_main --port 0`)
...
/// Best-effort project-root resolver for dev mode. The Cargo manifest sits at
/// `<project>/src/utils/scaled_point_viewer/frontend/src-tauri/Cargo.toml`, so
...
fn python_executable() -> &'static str {
    option_env!("SCALED_POINT_VIEWER_PYTHON").unwrap_or("python")
}
...
fn spawn_sidecar() -> Result<(Child, u16), String> {
    ...
    let mut child = Command::new(python)
        .args([
            "-m",
            "src.utils.scaled_point_viewer.backend.sidecar_main",
            "--port",
            "0",
            "--log-level",
            "warning",
...
pub fn run() {
    let (child, port) = match spawn_sidecar() {
        Ok(pair) => pair,
        Err(e) => {
            eprintln!("[scaled-point-viewer] sidecar startup failed: {e}");
            std::process::exit(1);
        }
    };
